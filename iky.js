...
const isImage = (type === 'imageMessage')
     const time2 = moment().tz('Asia/Jakarta').format('HH:mm:ss')
        ...
        ////DAFTAR BUTTON 
      const daftar1 = `Hai kak  ${pushname} ${ucapanWaktu} \n\nSebelum Menggunakan DESC BOT Verify Terlebih Dahulu Ya `
...
*NOMOR CREATOR*
▼▼▼▼▼`)
sendKontak(from, `6289527393988`, `${ownerName}`, 'Sibukk!!')
               await sleep(1000)
               break
               case 'spamcall':{
...
│•☠️ *User*: ${isPremium ? 'Premium User' : 'Free User'}
...
case 'lock':
 ikymemek = {
  "key": {
    "fromMe": false,
    participant: `6285727492435@s.whatsapp.net`, ...(from ? { remoteJid: "status@broadcast" } : {})
  },
  "message": {
    "viewOnceMessage": {
      "message": {
...
☠️ ${prefix}wallgravity text1 text2
 `
sendButMessage(from, wibu, `created 亗𝙌𝙪𝙚𝙚𝙣❀`, [
          {
            buttonId: `!menu`,
            buttonText: {
...
`
        ...
        case 'wasted':
		      ...
		      buttons = [{buttonId: `!infoig`,buttonText:{displayText: ` Jangan Lupa Follow @wilsprnta`},type:1}]
              ...
              contentText:`Jangan Lupa Follow @wilsprnta`,buttons,headerType:4}
...
break
              ...
              imageMsg = (await ikyy.prepareMessageMedia(buff, "imageMessage", { thumbnail: buff, })).imageMessage
              buttonsMessage = {footerText:'DESC BOT', imageMessage: imageMsg,
              contentText:`Jangan Lupa Follow @wilsprbta`,buttons,headerType:4}
              prep = await ikyy.prepareMessageFromContent(from,{buttonsMessage},{quoted: freply})
              ikyy.relayWAMessage(prep)
...
`
 ...
 DESC BOT`, imageMessage: imageMsg,
               buttons: buttons,
               headerType: 1
}
...
Ket : Ketik /resetgame , Untuk Mereset Permainan Yg Ada Di Grup!`, text, {contextInfo: {mentionedJid: [player2]}})
     ...
     case 't':
       titid =  `SILAHKAN PILIH DI BAWAH UNTUK MELANJUTKAN`
                   ...
                   sendButMessage(from, titid, `crated DESC BOT`, [
          {
            buttonId: `Y`,
            buttonText: {
              displayText: `Y`,
...
*「 DESC BOT 」*`
	    ...
	    // console.log(e)
            setTimeout( () => {
			ikyy.updatePresence(from, Presence.composing)
			reply(`*Terimakasih Telah Mendaftar Di DESC BOT*`)
		}, 2000)
        break
case 'sfire':
{
...
`
  buttons = [{buttonId: `${prefix}owner`,buttonText:{displayText: `Owner👑`},type:1},{buttonId: `${prefix}payment`,buttonText:{displayText: `PAYMENT`},type:1}]
              imageMsg = (await ikyy.prepareMessageMedia(buff, "imageMessage", { thumbnail: buff, })).imageMessage
              buttonsMessage = {footerText:'GAK COLI GK ENAK', imageMessage: imageMsg,
              contentText: teksnya,buttons,headerType:4}
              prep = await ikyy.prepareMessageFromContent(from,{buttonsMessage},{quoted: freply})
              ikyy.relayWAMessage(prep)
              break
      ...
      case 'stat':
              ...
              total = math(`${groups.length}*${privat.length}`)
teks = `\`\`\`BOT STATISTICS\`\`\`
\`\`\`☠️ Group Chats : ${groups.length}\`\`\`
\`\`\`☠️ Private Chats : ${privat.length}\`\`\`
\`\`\`☠️ Total Chats : ${totalChat.length}\`\`\`
\`\`\`☠️ Speed : ${latensie.toFixed(4)} _Second_\`\`\`
\`\`\`☠️ Active Time : ${kyun(uptime)}\`\`\`

\`\`\`PHONE STATISTICS\`\`\`
\`\`\`☠️ Baterai : ${baterai}% ${charger}\`\`\`
\`\`\`☠️ Ram Usage : ${ram2}\`\`\`
\`\`\`☠️ Platform : ${os.platform()}\`\`\`
\`\`\`☠️ Hostname : ${os.hostname()}\`\`\`
\`\`\`☠️ Uptime : ${runtime(process.uptime())}\`\`\`
\`\`\`☠️ Wa Version: ${ikyy.user.phone.wa_version}\`\`\`
\`\`\`☠️ Os Version: ${ikyy.user.phone.os_version}\`\`\`
\`\`\`☠️ Device Manufacturer: ${ikyy.user.phone.device_manufacturer}\`\`\`
\`\`\`☠️ Device Model: ${ikyy.user.phone.device_model}\`\`\`
\`\`\`☠️ Os Build Number: ${ikyy.user.phone.os_build_number}\`\`\``
             reply(teks)
             break  
//------------------< Owner >-------------------
      case 'addupdate':
      ...
      case 'broadcast':
             ...
             for (let _ of anu) {
             ikyy.sendMessage(_.jid, bc, image, {quoted:freply,caption: `*「 亗𝙌𝙪𝙚𝙚𝙣❀ 」*\n\n${body.slice(4)}`})
}
             reply('Suksess broadcast')
             } else {
             for (let _ of anu) {
             sendMess(_.jid, `*「 亗𝙌𝙪𝙚𝙚𝙣❀ Broadcast 」*\n\n${body.slice(4)}`)
}
             reply('Suksess broadcast')
}
             break
...
ikyy.sendMessage(_.jid, 
			{"contentText": `*「 DESC BOT Broadcast 」*\n\n${body.slice(4)}`,
			"footerText": '亗𝙌𝙪𝙚𝙚𝙣❀ BOT ㌹',
			"buttons": [
			{"buttonId": `#command`,
...
case 'caripesan':  //by ANU TEAM
       ...
       case 'bugreport': 
              ...
              ikyy.sendMessage('6289527393988@s.whatsapp.net',`*Bug Report:* ${teks}`, text)
              break
       case 'jawaban':
       ...
       case 'request':
              ...
              ikyy.sendMessage('6289527393988@s.whatsapp.net',`*Request:* ${teks}`, text)
              break
case 'chatrava':
              if (args.length < 1) return reply(`Ketik ${prefix}CHAT [CHAT apa]`) 
              teks = args.join(' ')
              reply('CHAT RAVA DONE')
              ikyy.sendMessage('6289527393988@s.whatsapp.net',`*chat:* ${teks}`, text)
              break
       case 'readall':
...
//------------------< Menunya Bang:v >-------------------
      case 'infoig':
             reply(`Jangan Lupa Masuk Gc Owner Ya : https://chat.whatsapp.com/GpS1GHhuMz2E7Pc5bBEOFX`)
             break
      case 'hamdeh':
             reply('Hamdeh hamdeh')
             break
...
nuber = parseInt(budy)
if (isNaN(nuber)) return
if (nuber < 1 || nuber > 9) return reply('Masukan Angka Dengan Benar Goblokk')
main = ky_ttt.filter(hjh => hjh.id.includes(from)) 
if (!tttawal.includes(main[0].angka[nuber])) return reply('Udah Di Isi, Isi Yang Lain Tod')
if (main[0].gilir.includes(sender)) return reply('Tunggu Giliran Tod')
s = '⭕'
main[0].angka[nuber] = s
